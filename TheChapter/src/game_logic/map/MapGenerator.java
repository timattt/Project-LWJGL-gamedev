...
public abstract class MapGenerator {
	...
	protected final float[][] readHeightMap(File file, int width, int height) throws IOException {
	...
	protected final Tile[][] genTiles(float[][] mapZ) {
		Tile[][] result = new Tile[(mapZ.length - 1)
				/ (TileSizeHandler.instance.getTileCuts() + 1)][(mapZ[0].length - 1)
						/ (TileSizeHandler.instance.getTileCuts() + 1)];
