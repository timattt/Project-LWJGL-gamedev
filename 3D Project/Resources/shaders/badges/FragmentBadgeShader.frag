...
const float interval_alpha_start = 0.25;
const float interval_alpha_end = 1;

void setupColours(Material material, vec2 textCoord)
{			
		vec4 texture_color = texture(frag_texture.texture_sampler, texturePos); 
    	
        ambientC = texture_color + vec4(color.xyz, (interval_alpha_end - interval_alpha_start) * alpha + interval_alpha_start);
        diffuseC = ambientC;
        speculrC = ambientC;
